...
function Entity.on_ground(self)
    return G.game.level:has_collision(self.cx, self.cy-1) and self.yr == 0 and self.dy <= 0
end
...
function Entity.on_pre_step_y(self)
    -- Down
    if self.has_collisions and self.yr < 0.0 and G.game.level:has_collision(self.cx, self.cy - 1) then
        self.dy = 0
        self.yr = 0
